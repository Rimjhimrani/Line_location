...
# --- 1. EXACT STYLE DEFINITIONS ---
styles = getSampleStyleSheet()

# Bin Label Styles (Strictly as provided by you)
...
# Rack Label Styles (Strictly as provided by you)
...
def parse_dimensions(dim_str):
    """Parses L x W strings to numbers"""
...
def extract_store_location_data_from_excel(row_data):
    """Exact helper provided for Bin Labels"""
    ...
    def get_clean_value(possible_names):
        ...
        return ""
    return [
        get_clean_value(['ST. NAME (Short)', 'Station Name']), get_clean_value(['Store Location', 'STORELOCATION']),
        get_clean_value(['ABB ZONE']), get_clean_value(['ABB LOCATION']), get_clean_value(['ABB FLOOR']),
        get_clean_value(['ABB RACK NO']), get_clean_value(['ABB LEVEL IN RACK'])
    ]

def extract_location_values(row):
    """Helper to extract the 7 location keys"""
    return [str(row.get(c, '')) for c in ['Bus Model', 'Station No', 'Rack', 'Rack No 1st', 'Rack No 2nd', 'Level', 'Cell']]

def format_part_no_v2(part_no):
    """HUGE font for Rack Labels only (Size 34/40)"""
    part_no = str(part_no)
...
def automate_location_assignment(df, base_rack_id, levels, cells_per_level, bin_info_map, status_text=None):
    ...
    for st_no, st_group in df_p.groupby('Station No', sort=True):
        ...
        df_g = st_group.copy()
        df_g['bin_area'] = df_g['Container'].map(lambda x: bin_info_map.get(x, {}).get('dims', (0,0))[0] * bin_info_map.get(x, {}).get('dims', (0,0))[1])
        df_g['bins_per_cell'] = df_g['Container'].map(lambda x: bin_info_map.get(x, {}).get('capacity', 1))
        container_groups = sorted(df_g.groupby('Container'), key=lambda x: x[1]['bin_area'].iloc[0], reverse=True)
        st_cells_needed = sum([math.ceil(len(c_df)/c_df['bins_per_cell'].iloc[0]) for _, c_df in container_groups])
        r_needed = math.ceil(st_cells_needed / (len(levels) * cells_per_level))
        st_cells = []
        for r_idx in range(1, r_needed + 1):
            r_str = f"{r_idx:02d}"
        ...
        for _, cont_df in container_groups:
            ...
            for i in range(0, len(parts), cap):
                chunk = parts[i:i+cap]
                if ptr < len(st_cells):
                    for p in chunk: p.update(st_cells[ptr])
                    ptr += 1
        for i in range(ptr, len(st_cells)):
            ...
            empty.update(st_cells[i]); final_assigned.append(empty)
        final_assigned.extend([p for _, c_df in container_groups for p in c_df.to_dict('records') if 'Rack No 1st' in p])
...
def assign_sequential_location_ids(df):
    ...
    for _, row in df_parts.iterrows():
        ...
        counters[key] = counters.get(key, 0) + 1; seq.append(counters[key])
    df_parts['Cell'] = seq
...
# --- 4. EXACT PDF GENERATION FUNCTIONS ---
...
def generate_bin_labels(df, mtm_models, progress_bar=None, status_text=None):
    ...
    total = len(df_filtered); all_elements = []
    ...
    for i, row in enumerate(df_filtered.to_dict('records')):
        ...
        part_no, desc, qty_bin = str(row.get('Part No')), str(row.get('Description')), str(row.get('Qty/Bin',''))
        content_width = CONTENT_BOX_WIDTH - 0.2*cm
        
        # BIN LABEL TABLE: USES bin_bold_style (Size 14) to avoid "Huge" overlapping text
        main_table = Table([
        ...
        store_inner = Table([extract_store_location_data_from_excel(row)], colWidths=[(content_width*2/3)/7]*7, rowHeights=[0.5*cm])
        store_inner.setStyle(TableStyle([('GRID', (0,0),(-1,-1), 1.2, colors.black), ('FONTSIZE', (0,0),(-1,-1), 9), ('ALIGN', (0,0),(-1,-1), 'CENTER'), ('VALIGN', (0,0),(-1,-1), 'MIDDLE')]))
        ...
        line_inner = Table([line_vals], colWidths=[(content_width*2/3)/7]*7, rowHeights=[0.5*cm])
        line_inner.setStyle(TableStyle([('GRID', (0,0),(-1,-1), 1.2, colors.black), ('FONTSIZE', (0,0),(-1,-1), 9), ('ALIGN', (0,0),(-1,-1), 'CENTER'), ('VALIGN', (0,0),(-1,-1), 'MIDDLE')]))
        ...
        qr_img = generate_qr_code_image(f"Part No: {part_no}\nLoc: {row.get('Level')}{row.get('Cell')}")
        mtm_table = None
        if mtm_models:
            qty_v, bus_m = str(row.get('Qty/Veh', '')), str(row.get('Bus Model', '')).strip().upper()
...
def generate_rack_labels_v2(df, progress_bar=None, status_text=None):
    """EXACT Rack Label Design with Huge Font 34/40"""
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=A4, topMargin=1*cm, bottomMargin=1*cm, leftMargin=1.5*cm, rightMargin=1.5*cm)
    df_parts = df[df['Part No'].str.upper() != 'EMPTY'].copy()
    elements = []; total = len(df_parts)
    for i, row in enumerate(df_parts.to_dict('records')):
        if progress_bar: progress_bar.progress(int(((i+1)/total)*100))
        if i > 0 and i % 4 == 0: elements.append(PageBreak())
        part_table = Table([['Part No', format_part_no_v2(row.get('Part No'))], ['Description', Paragraph(str(row.get('Description')), desc_style)]], colWidths=[4*cm, 11*cm], rowHeights=[1.9*cm, 2.1*cm])
        ...
        location_table = Table(location_data, colWidths=[4*cm]+[w*(11*cm)/11.1 for w in [1.7, 2.9, 1.3, 1.2, 1.3, 1.3, 1.3]], rowHeights=0.9*cm)
        ...
        for j, col in enumerate(['#E9967A','#ADD8E6','#90EE90','#FFD700','#ADD8E6','#E9967A','#90EE90']):
            loc_style.append(('BACKGROUND', (j+1, 0), (j+1, 0), colors.HexColor(col)))
...
def generate_rack_list_pdf(df, base_rack_id, top_logo_file, progress_bar=None):
    ...
    elements = []; df_clean = df[df['Part No'].str.upper() != 'EMPTY'].copy()
...
def main():
    ...
    if uploaded_file:
        ...
        if container_col and station_col:
            ...
            # Dictionary comprehension using strictly defined functions
            bin_info_map = {}
            for cont in sorted(df[container_col].dropna().unique()):
                st.sidebar.markdown(f"**{cont}**")
                d_in = st.sidebar.text_input(f"Dim {cont}", "600x400")
                c_in = st.sidebar.number_input(f"Cap {cont}", 1)
                bin_info_map[cont] = {'dims': parse_dimensions(d_in), 'capacity': c_in}
            ...
            if st.button("🚀 Generate PDF", type="primary"):
                ...
                if output_type == "Rack Labels":
                    pdf_buf = generate_rack_labels_v2(df_final, prog)
