...
def find_required_columns(df):
    ...
    # Priority for Long Station Name
    ...
    return {
        ...
        'Station Name': cols.get(station_name_long_key), # Full Name for List
    ...
    for station_no, station_group in df_processed.groupby('Station No', sort=True):
        ...
        for i in range(current_cell_ptr, len(station_available_cells)):
            empty_label = {
                ...
                'Station No': station_no, 'Station Name': station_group['Station Name'].iloc[0], 
                'Container': ''
            }
...
def generate_by_rack_type(df, base_rack_id, rack_templates, container_configs, status_text=None):
    ...
    rename_dict = {
        ...
        req['Station Name']: 'Station Name', req['Container']: 'Container'
    }
...
def extract_store_location_data_from_excel(row):
    ...
    # Specifically using Short Name for store location labels
    short_st_name = get_clean_value(['ST. NAME (Short)', 'ST.NAME (Short)', 'ST NAME (Short)', 'Short Name']) 
    
    return [short_st_name, store_location, zone, location, floor, rack_no, level_in_rack]
...
def generate_bin_labels(df, mtm_models, progress_bar=None, status_text=None):
    ...
    for i, row in enumerate(df_filtered.to_dict('records')):
        ...
        qr_data = f"Part No: {part_no}\nDesc: {desc}\nQty/Bin: {qty_bin}\nStore Loc: {'|'.join(map(str,store_loc_raw))}\nLine Loc: {'|'.join(map(str,line_loc_raw))}"
        ...
        bottom_row = Table([[None, None, qr_image or "", ""]], colWidths=[mtm_width, gap_width, qr_width, content_width-mtm_width-gap_width-qr_width], rowHeights=[2.5*cm])
...
def generate_rack_list_pdf(df, base_rack_id, top_logo_file, top_logo_w, top_logo_h, fixed_logo_path, progress_bar=None, status_text=None):
    ...
    for i, ((station_no, rack_key), group) in enumerate(grouped):
        ...
        # Using long Station Name for the master header table
        station_name_long = str(first_row.get('Station Name', ''))
        ...
        master_data = [
            [Paragraph("STATION NAME", ParagraphStyle('H', fontName='Helvetica-Bold', fontSize=12)), Paragraph(station_name_long, master_value_style_left), Paragraph("STATION NO", ParagraphStyle('H', fontName='Helvetica-Bold', fontSize=13)), Paragraph(str(station_no), master_value_style_center)],
        ...
        if has_zone:
            header_row.insert(0, "ZONE"); col_widths = [2.0*cm, 1.3*cm, 4.0*cm, 8.2*cm, 3.5*cm, 2.5*cm, 6.0*cm]
            
        data_rows = [header_row]
        group_sorted = group.sort_values(by=['Level', 'Cell'])
        table_style_cmds = [('GRID', (0,0), (-1,-1), 1, colors.black), ('BACKGROUND', (0,0), (-1,0), colors.HexColor("#F4B084")), ('ALIGN', (0,0), (-1,-1), 'CENTER'), ('VALIGN', (0,0), (-1,-1), 'MIDDLE')]
        ...
        for idx, row in enumerate(group_sorted.to_dict('records')):
            ...
            row_data = [str(idx+1), row['Part No'], Paragraph(row['Description'], rl_cell_left_style), row['Container'], row.get('Qty/Bin',''), loc_str]
            if has_zone: row_data.insert(0, str(row.get('Zone','')))
            data_rows.append(row_data)

        elements.append(Table(data_rows, colWidths=col_widths, style=table_style_cmds)); elements.append(Spacer(1, 0.2*cm))
        ...
        footer_table = Table([[Paragraph(f"<i>Creation Date: {datetime.date.today().strftime('%d-%m-%Y')}</i>", rl_cell_left_style), fixed_logo_img]], colWidths=[20*cm, 7.7*cm])
    ...
    if uploaded_file:
        ...
        if req_cols['Container'] and req_cols['Station No']:
            ...
            if generation_method == "By Cell Dimension":
                ...
                bin_rules = {c: {'dims': (600,400), 'capacity': st.sidebar.number_input(f"Cap: {c}", 1, 100, 1)} for c in unique_c}
            else:
