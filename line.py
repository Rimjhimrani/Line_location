...
def generate_by_rack_type(df, base_rack_id, rack_templates, status_text=None):
    ...
    # Logic uses the first configured template
        ...
        for cont_type, parts_group in station_group.groupby('Container', sort=True):
            ...
            for part in all_parts:
                ...
                part.update({
                    ...
                    'Rack Type Header': f"{template_name} ({dim_val})"
                })
...
def assign_sequential_location_ids(df):
    ...
    return df_parts_only

def generate_summary_table(df):
    if df.empty: return pd.DataFrame()
    # Count unique Rack Keys (01, 02...) per Station and Rack Type
    summary = df.groupby(['Station No', 'Rack Type Header'])['Rack Key'].nunique().reset_index()
    pivot_df = summary.pivot(index='Station No', columns='Rack Type Header', values='Rack Key').fillna(0).astype(int)
...
def extract_store_location_data(row):
    def get_val(names):
        ...
        return ""
    return [get_val(['Store Location']), get_val(['ST. NAME (Short)', 'Station Name Short']), get_val(['Zone', 'ABB ZONE']), 
            get_val(['Location']), get_val(['Floor']), get_val(['Rack No']), get_val(['Level'])]
...
def generate_rack_labels(df, progress_bar=None):
    ...
    elements = []
    df_p = df.sort_values(by=['Station No', 'Rack No 1st', 'Rack No 2nd', 'Level', 'Cell'])
    total = len(df_p)
    for i, row in enumerate(df_p.to_dict('records')):
        ...
        loc_vals = ['Line Location'] + [str(row.get(c,'')) for c in ['Bus Model', 'Station No', 'Rack', 'Rack No 1st', 'Rack No 2nd', 'Level', 'Cell']]
...
def main():
    ...
    uploaded_file = st.file_uploader("Upload Your Data (Excel/CSV)", type=['xlsx', 'xls', 'csv'])
    ...
    if uploaded_file:
        ...
        if req['Container'] and req['Station No']:
            ...
            r_name = st.sidebar.text_input("Rack Type Name", "Rack-A")
            r_dim = st.sidebar.text_input("Rack Dimension", "2400x800")
            r_levels = st.sidebar.multiselect("Active Levels", ['A','B','C','D','E','F'], default=['A','B','C','D'])
            
            st.sidebar.caption("Bins per Shelf (Capacity):")
            caps = {c: st.sidebar.number_input(f"{c} capacity", 1, 50, 4) for c in unique_c}
