...
# --- EXACT STYLE DEFINITIONS FROM YOUR SNIPPET ---
...
# --- EXACT PDF DESIGN FUNCTIONS FROM YOUR SNIPPETS ---
...
def generate_bin_labels(df, mtm_models, progress_bar=None, status_text=None):
    ...
    for i, row in enumerate(df_filtered.to_dict('records')):
        ...
        rack_key = f"ST-{row.get('Station No')} / Rack {row.get('Rack No 1st')}{row.get('Rack No 2nd')}"; label_summary[rack_key] = label_summary.get(rack_key,0)+1
    ...
    if uploaded_file:
        ...
        if container_col and station_col:
            ...
            if st.button("🚀 Generate PDF", type="primary"):
                df_assigned = generate_station_wise_assignment(df, base_rack_id, levels, num_cells, bin_info_map)
