        ...
        for cont_type, parts_group in station_group.groupby('Container', sort=True):
            ...
            for part in all_parts:
                ...
                part.update({
                    ...
                    'Rack Type Summary Header': f"{template_name} ({dim_val})"
                })
...
def generate_rack_summary_table(df):
    ...
    # Filter out empty rows if any for counting racks
    df_active = df[df['Part No'].astype(str).str.upper() != 'EMPTY'].copy()
    
    # Count unique Rack Keys per Station and Rack Type
    summary = df_active.groupby(['Station No', 'Rack Type Summary Header'])['Rack Key'].nunique().reset_index()
    
    # Pivot to have Rack Types as columns
