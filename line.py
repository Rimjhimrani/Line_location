...
# --- CORE LINE AUTOMATION LOGIC (REQUESTED VERSION) ---
...
def extract_store_location_data_from_excel(row_data):
    ...
    return [get_clean_value(['ST. NAME (Short)']), get_clean_value(['Store Location']), get_clean_value(['ABB ZONE']), 
            get_clean_value(['ABB LOCATION']), get_clean_value(['ABB FLOOR']), get_clean_value(['ABB RACK NO']), 
            get_clean_value(['ABB LEVEL IN RACK'])]

# --- PDF GENERATORS (EXACT DESIGN PRESERVED) ---
...
def generate_rack_labels_v1(df, progress_bar=None, status_text=None):
    ...
    for i, (location_key, group) in enumerate(df_grouped):
        ...
        location_data = [[Paragraph('Line Location', location_header_style)] + [Paragraph(str(v), get_dynamic_location_style(v)) for v in loc_vals]]
...
def generate_bin_labels(df, mtm_models, progress_bar=None, status_text=None):
    ...
    for i, row in enumerate(df_filtered.to_dict('records')):
        ...
        main_table = Table([["Part No", Paragraph(str(row.get('Part No')), bin_bold_style)], ["Description", Paragraph(str(row.get('Description'))[:47], bin_desc_style)], ["Qty/Bin", Paragraph(str(row.get('Qty/Bin','')), bin_qty_style)]], colWidths=[content_width/3, content_width*2/3], rowHeights=[0.9*cm, 1.0*cm, 1.0*cm])
        ...
        store_row = Table([[Paragraph("Store Location", bin_desc_style), store_inner]], colWidths=[content_width/3, content_width*2/3])
        line_inner = Table([extract_location_values(row)], colWidths=[(content_width*2/3)/7]*7, rowHeights=[0.5*cm])
        line_inner.setStyle(TableStyle([('GRID', (0,0),(-1,-1), 1.2, colors.black), ('FONTSIZE', (0,0),(-1,-1), 9), ('ALIGN', (0,0),(-1,-1), 'CENTER')]))
        line_row = Table([[Paragraph("Line Location", bin_desc_style), line_inner]], colWidths=[content_width/3, content_width*2/3])
        qr_img = generate_qr_code_image(f"P:{row.get('Part No')}\nL:{row.get('Level')}{row.get('Cell')}")
        mtm_table = Table([mtm_models, [""]*len(mtm_models)], colWidths=[3.6*cm/len(mtm_models)]*len(mtm_models), rowHeights=[0.75*cm]*2) if mtm_models else Spacer(1,1)
        bottom = Table([[mtm_table, "", qr_img, ""]], colWidths=[3.6*cm, 1.0*cm, 2.5*cm, content_width-7.1*cm], rowHeights=[2.5*cm])
        all_elements.extend([main_table, store_row, line_row, Spacer(1, 0.2*cm), bottom, PageBreak()])
    doc.build(all_elements, onFirstPage=draw_border, onLaterPages=draw_border); buffer.seek(0)
    return buffer, {"Total": len(df_filtered)}
...
def main():
    ...
    if uploaded_file:
        ...
        if container_col and station_col:
            levels = st.sidebar.multiselect("Levels", options=['A','B','C','D','E','F','G','H','I'], default=['A','B','C','D'])
            ...
            if st.button("🚀 Generate PDF", type="primary"):
                ...
                if output_type == "Rack Labels":
                    pdf_buf, summary_df = generate_rack_labels_v2(df_final, prog) if rack_format == "Single Part" else generate_rack_labels_v1(df_final, prog)
                elif output_type == "Bin Labels":
                    pdf_buf, _ = generate_bin_labels(df_final, mtm_models, prog)
                    summary_df = df_final[df_final['Part No'] != 'EMPTY'].groupby(['Station No']).size().reset_index(name='Total Parts')
                else:
                ...
                # Group specifically by station and unique racks to show calculation
